...
def style_transfer(name, content_image, style_image, image_size, style_size, content_layer, content_weight,
    ...
    for t in range(200):
        ...
        optimizer.zero_grad()
        contentLoss = content_loss.forward(content_weight, feats[content_layer], content_target)
        
        styleLoss = style_loss.forward(feats, style_layers, style_targets, style_weights)
    
        tvLoss = tv_loss.forward(img_var, tv_weight)
